...
Write a function that, given an array of integer arrays as input, calculates the total sum of all the items in the array.
...
This function should num1 remove any items that are not numbers or are not divisible by five.
  ...
  input.map((item, index) => {
    ...
    item.map(Item => {
      if (typeof (Item) === 'number' && Item % 5 === 0) {
        let newItem = Math.pow(2, Item);
        divisible[index].push(newItem);
      }
    });
  });
  return divisible;
...
let findShortest = (data) => {
  ...
  let test = data.sort((num1, num2) => {
    if (parseInt(num1.height) < parseInt(num2.height)) {
      return -1;
    }
  });
