...
const screenForNames = (arr) => {
  let regex = /^(Mr.|Mrs.|Ms.|Dr.)(\s)(\w{1,})/g;

  let newArray = arr.filter(item => {
    console.log(item, regex.test(item), item.match(regex));
    return (item, regex.test(item), item.match(regex));
  });
  return (newArray)
  // Solution code here...
};
...
Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the array at those locations. For instance helpCheck(R1, C1, R2, C2, R3, C3).
...
const detectTicTacToeWin = (board) => {
  ...
  let R1 = board[0];
  let C1 = [board[0][0], board[1][0], board[2][0]];
  let R2 = board[1];
  let C2 = [board[0][1], board[1][1], board[2][1]];
  let R3 = board[2];
  let C3 = [board[0][2], board[1][2], board[2][2]];
  let D1 = [board[0][0], board[1][1], board[2][2]];
  let D2 = [board[0][2], board[1][1], board[2][0]];
  let total = [R1, R2, R3, C1, C2, C3, D1, D2];
  console.log(total);
